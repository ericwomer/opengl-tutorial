...
// fun fact: these aren't actually random
float rand(float p) {
...
vec3 pixel(vec2 uv) {
	...
	float t = time* 0.15;
	float down = PI / 2.0;
	rd.yz = rotate(rd.yz, time*.5 * cos(time*.6)*.00343);
	rd.xz = rotate(rd.xz, 0.0);
...
void rota(inout vec2 p,float theta){
	vec2 v = p;
	p.x = cos(theta)*v.x - sin(theta)*v.y;
	p.y = sin(theta)*v.x + cos(theta)*v.y;
}
...
void main() {
	...
	gl_FragColor.b *= 0.3;
	gl_FragColor.g *= 0.23 + .02*cos(time);
	gl_FragColor.r *= 0.05;
}
